...
export async function generateDashboardData(input: {
  project: string;
  vaultRoot: string;
}): Promise<{ jsonPath: string; data: DashboardData }> {
  const project = slugifyProject(input.project);
  const data = await collectDashboardData(input.vaultRoot, project);
  const jsonPath = await writeJsonArtifact(input.vaultRoot, project, "dashboard", "dashboard-data.json", data);
  return { jsonPath, data };
}

export async function collectDashboardData(vaultRoot: string, projectInput: string): Promise<DashboardData> {
