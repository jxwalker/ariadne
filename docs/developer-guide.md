...
That command installs dependencies, installs Playwright Chromium, runs checks, builds the runner, refreshes the control plane, verifies the console, captures a browser screenshot, and prints the current operator handoff. It is repeatable after `git pull --ff-only`.

The underlying npm script remains available for CI or shells that cannot execute the wrapper:
...
- `src/liveAdapterCutoverAudit.ts`: final non-mutating cutover audit before placeholder commands can be replaced by target adapters; it now consumes operator-evidence audit state as a blocking gate.
...
Use `mutation-readiness` to bind those pieces into one reviewable artifact before adding a live adapter. Prefer target-specific planning wrappers such as `github-mutation-plan`, `notebooklm-mutation-plan`, `gsd2-mutation-plan`, `hermes-cron-mutation-plan`, `openscorpion-mutation-plan`, and `deployment-mutation-plan` when they exist, because they force the target's required identifiers into the reviewed scope. The plan must keep `execute=false`, include a post-action verification command, and pass `mutation-readiness-audit`; live commands belong in later adapters only after the readiness plan has been reviewed. When execution is allowed, prefer the matching target-specific execute wrapper such as `github-mutation-execute` or `deployment-mutation-execute` so the command itself enforces the target guard before delegating to the audited execution path. Before replacing placeholder commands with a real external adapter, run `live-adapter-readiness`, `live-adapter-next-actions`, `live-adapter-approval-pack`, `live-adapter-approval-review-audit`, `live-adapter-dossier`, `live-adapter-review-session`, `live-adapter-evidence-templates`, `live-adapter-operator-evidence-workplan`, `live-adapter-operator-evidence-queue`, `live-adapter-operator-evidence-workspace`, `live-adapter-operator-evidence-assist`, `live-adapter-operator-evidence-check-all --source workspace`, `live-adapter-operator-evidence-audit`, and `live-adapter-cutover-audit`; use `--target <target>` on cutover audits and review sessions when you are moving one adapter through the gates, and use `live-adapter-operator-evidence-check` for a single target after filling its workspace file. Keep the adapter blocked until the target has complete imported operator evidence, an accepted audit-current `live-adapter-approval-review`, an audit-passed plan, passed dry-run evidence, passed target-guarded execution evidence, a generated target dossier, and a passing cutover target. A packet review is not a mutation approval; it only says the operator has reviewed the packet itself. A review session is also not approval; it only consolidates the evidence the operator needs to inspect. Evidence templates, workspaces, and assist packets are collection aids and must never be counted as proof until an operator fills the real evidence file with verified observations. The single-target preflight, batch preflight, and queue are also not proof; they only validate and order filled workspace files before import.
