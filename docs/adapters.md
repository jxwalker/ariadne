...
`operator-next` is the short human-facing wrapper for that packet. It refreshes the current packet, refreshes the roadmap completion audit after the packet writes its target artifacts, refreshes the console, then prints the console path, packet path, exact `operator-evidence.md` file to fill, current missing section, section start guidance, record location, preflight expectation, one-section guide command, the preflight command, and the import command that remains gated until after human verification.
