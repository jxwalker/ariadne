...
- Use `local-runtime-probe --canary` to record Hermes, Ollama, DS4/OpenAI-compatible, LM Studio, and Atlas endpoint health before routing evaluation workloads. The console reads these runtime probe artifacts as infrastructure evidence; configured non-loopback endpoint URLs are redacted in persisted probe artifacts. A degraded canary is evidence to tune the endpoint or model choice, not approval to mutate services.
