...
- short `operator-next` handoff that refreshes the current packet and console, then prints the console path, packet path, fill file, current missing section, section start guidance, record location, preflight expectation, one-section guide command, preflight command, and later import command without importing evidence or granting approval
- focused `operator-section` handoff that turns the current missing operator-evidence section into a one-section guide with start refs, GBrain advisory queries, evidence file, and preflight command without importing evidence or granting approval
